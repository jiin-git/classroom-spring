...
function addLectureList(lecture) {
    ...
    addLectureInfo(lectureBasicInfo);
...
function addLectureInfo(lectureBasicInfo) {
    ...
    lectureInfo.innerHTML += `
           ...
           <p class="card-text gmarketSansFont">정원: ${lectureBasicInfo.personnel - lectureBasicInfo.remainingPersonnel} / ${lectureBasicInfo.personnel}</p>
           <p class="card-text"><small class="text-body-secondary">${lectureBasicInfo.lectureStatus}</small></p>
           <a class="btn btn-secondary" href="/instructor/lectures/${lectureBasicInfo.id}">강의정보</a>`;
}
function addProfileImage(profileImage, lectureId) {
    const profileId = document.getElementById(`lecture-profile-${lectureId}`);
